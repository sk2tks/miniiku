...
class UploadHandlerComponent extends Component{
    ...
    function __construct( ComponentCollection $collection, $options = null, $error_messages = null ) {
        $this->options = array(
	...
	public function setThumbSize($width=80, $height=80){
    ...
    public function post($print_response = true) {
		...
		//$this->log($_FILES[$this->options['param_name']]);
	...
	public function delete_file_recursive($dir, $target){
		if ( !file_exists($dir) ) {
			return;
		}
		$dhandle = opendir($dir);
		...
		if ( $dhandle ) {
			while( false !== ($fname = readdir($dhandle)) ) { 
				if ( is_dir($dir . DS .$fname) ) {
					if ( ($fname != '.') && ($fname != '..') ) {
						$this->delete_file_recursive($dir . DS .$fname, $target);
					}
				} else {
					$file = $dir . DS .$fname;
					if ( is_file($file) && basename($file) == $target ) {
						$ret = unlink($file);
					//	$this->log(array($ret, $file));			
					}
				}
			}
			closedir($dhandle);
	...
	public function move_upload_files($directory = '', $filename = null, $newname=null){
		if(empty($filename)  || empty($directory)) return null;
		
		if(!empty($newname)){
			$ext = substr($filename, strrpos($filename, '.') + 1);
			$new_name = sprintf("%s.%s",$newname , $ext);
		}else{
			$new_name = $filename;
		}
		
		$moved = false;
		$image_versions = array_keys($this->options['image_versions']);
		foreach($image_versions as $size){
			if($size == ''){
				$temp_file = TEMP_UPLOAD_DIR.$filename;
				$new_file = $directory . DS . $new_name;
			}else{
				$temp_file = TEMP_UPLOAD_DIR.$size . DS . $filename;
				$target_dir = $directory . DS . $size;
				$this->log(array($target_dir, $temp_file));
				if(!is_dir($target_dir)){
					mkdir($target_dir);
					chmod( $target_dir, 0777);
				}
				$new_file = $target_dir . DS . $new_name;
			}
			
			if(is_file($temp_file)){
				$ret = @rename($temp_file, $new_file );
				$moved = true;
			}
		}
